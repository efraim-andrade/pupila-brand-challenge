...
const setupStoreMock = (
  ...
  (useAppStore.useAppStore as unknown as jest.Mock).mockImplementation(
    (selector) => {
...
describe('ConfigurationModal', () => {
  ...
  describe('group items', () => {
    ...
    it('renders group icon', () => {
      ...
      expect(
        screen.getByRole('button', { name: /Edit Group 1/i })
      ).toBeInTheDocument();
    });
  });
  ...
  describe('edit item', () => {
    it('shows edit button on hover', async () => {
      ...
      expect(
        screen.getByRole('button', { name: 'Edit Group 1' })
      ).toBeInTheDocument();
    });
