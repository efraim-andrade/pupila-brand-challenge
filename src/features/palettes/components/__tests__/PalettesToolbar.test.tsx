...
describe('PalettesToolbar', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders tag buttons when tags are provided', () => {
      ...
      expect(
        screen.getByRole('button', { name: 'Tag One' })
      ).toBeInTheDocument();
      expect(
        screen.getByRole('button', { name: 'Tag Two' })
      ).toBeInTheDocument();
    });
  });
  ...
  describe('count label', () => {
    it('shows "N palettes" when no filter is active', () => {
      render(
        <PalettesToolbar {...defaultProps} totalCount={5} filteredCount={5} />
      );
    ...
    it('shows singular "palette" when count is 1', () => {
      render(
        <PalettesToolbar {...defaultProps} totalCount={1} filteredCount={1} />
      );
    ...
    it('shows "X of Y" when filter is active', () => {
      render(
        <PalettesToolbar
          ...
          totalCount={10}
          filteredCount={3}
          filter={{ ...defaultFilter, search: 'test' }}
        />
      );
  ...
  describe('search', () => {
    it('calls onFilterChange with search value when typing', async () => {
      ...
      render(
        <PalettesToolbar {...defaultProps} onFilterChange={onFilterChange} />
      );
    ...
    it('populates search input with existing filter value', () => {
      render(
        <PalettesToolbar
          {...defaultProps}
          filter={{ ...defaultFilter, search: 'existing' }}
        />
      );

      expect(screen.getByPlaceholderText('Search palettes…')).toHaveValue(
        'existing'
      );
    });
  });
    ...
    it('pre-selects group from filter', () => {
      render(
        <PalettesToolbar
          {...defaultProps}
          groups={groups}
          filter={{ ...defaultFilter, groupId: 'g2' }}
        />
      );
    ...
    it('removes tag when already active', async () => {
      ...
      render(
        <PalettesToolbar
          {...defaultProps}
          tags={tags}
          filter={{ ...defaultFilter, tagIds: ['t1'] }}
          onFilterChange={onFilterChange}
        />
      );
