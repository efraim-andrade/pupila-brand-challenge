...
describe('imagesSlice — initial state', () => {
  ...
  it('starts with the default imagesFilter', () => {
    const store = makeStore();
    expect(store.getState().imagesFilter).toEqual({
      search: '',
      groupId: null,
      tagIds: [],
    });
...
describe('addImage', () => {
  ...
  it('calls imageDB.put with the created image', () => {
    ...
    expect(mockedImageDB.put).toHaveBeenCalledWith(
...
describe('updateImage', () => {
  it('merges updates into the matching image', () => {
    ...
    store
      .getState()
      .hydrateImages([makeImage({ id: 'img-1', name: 'Old Name' })]);
  ...
  it('refreshes updatedAt on the matching image', () => {
    ...
    store
      .getState()
      .hydrateImages([
        makeImage({ id: 'img-1', updatedAt: '2024-01-01T00:00:00.000Z' }),
      ]);
  ...
  it('calls imageDB.put with the updated image', () => {
    ...
    expect(mockedImageDB.put).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'img-1', name: 'New' })
    );
  });
});
...
describe('addImageComment', () => {
  ...
  it('does not mutate non-matching images', () => {
    ...
    store
      .getState()
      .hydrateImages([makeImage({ id: 'img-1', comments: [] }), imageB]);
...
describe('updateImageComment', () => {
  it('updates the text of the matching comment', () => {
    ...
    store
      .getState()
      .hydrateImages([makeImage({ id: 'img-1', comments: [comment] })]);
  ...
  it('refreshes comment.updatedAt', () => {
    ...
    expect(typeof store.getState().images[0].comments[0].updatedAt).toBe(
      'string'
    );
  });
...
describe('deleteImageComment', () => {
  ...
  it('leaves non-matching comments in the array', () => {
    ...
    store
      .getState()
      .hydrateImages([
        makeImage({ id: 'img-1', comments: [commentA, commentB] }),
      ]);
...
describe('resetImagesFilter', () => {
  it('resets the filter to the default values', () => {
    ...
    store
      .getState()
      .setImagesFilter({ search: 'sunset', groupId: 'g-1', tagIds: ['t-1'] });
