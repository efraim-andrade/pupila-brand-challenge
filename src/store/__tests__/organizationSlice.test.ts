...
describe('addGroup', () => {
  ...
  it('assigns an id from nanoid to the created group', () => {
    ...
    const created = store
      .getState()
      .addGroup({ name: 'Design', type: 'image' });
  ...
  it('merges the provided fields into the created group', () => {
    ...
    const created = store
      .getState()
      .addGroup({ name: 'Design', type: 'palette', color: '#ff0000' });
...
describe('updateGroup', () => {
  it('merges updates into the matching group', () => {
    ...
    expect(store.getState().groups[0]).toEqual(
      expect.objectContaining({ id: 'g-1', name: 'New Name', type: 'image' })
    );
  });
  ...
  it('calls groupDB.put with the updated group', () => {
    ...
    expect(mockedGroupDB.put).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'g-1', name: 'New' })
    );
  });
});
...
describe('updateTag', () => {
  it('merges updates into the matching tag', () => {
    ...
    store
      .getState()
      .hydrateOrganization([], [makeTag({ id: 't-1', name: 'Old Name' })]);

    store.getState().updateTag('t-1', { name: 'New Name' });

    expect(store.getState().tags[0]).toEqual(
      expect.objectContaining({ id: 't-1', name: 'New Name' })
    );
  });
  ...
  it('calls tagDB.put with the updated tag', () => {
    ...
    store
      .getState()
      .hydrateOrganization([], [makeTag({ id: 't-1', name: 'Old' })]);

    store.getState().updateTag('t-1', { name: 'New' });

    expect(mockedTagDB.put).toHaveBeenCalledWith(
      expect.objectContaining({ id: 't-1', name: 'New' })
    );
  });
});
