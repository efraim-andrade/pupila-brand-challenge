...
describe('palettesSlice — initial state', () => {
  ...
  it('starts with the default palettesFilter', () => {
    const store = makeStore()
    expect(store.getState().palettesFilter).toEqual({ search: '', groupId: null, tagIds: [] })
  })
})
...
describe('addPalette', () => {
  it('prepends the new palette to the front of the palettes array', () => {
    ...
    store.getState().addPalette({ name: 'New Palette', colors: [], groupId: null, tagIds: [], comments: [] })
  ...
  it('assigns the id returned by nanoid', () => {
    const store = makeStore()
    store.getState().addPalette({ name: 'X', colors: [], groupId: null, tagIds: [], comments: [] })
  ...
  it('calls paletteDB.put with the created palette', () => {
    const store = makeStore()
    store.getState().addPalette({ name: 'Warm Tones', colors: [], groupId: null, tagIds: [], comments: [] })

    expect(mockedPaletteDB.put).toHaveBeenCalledWith(expect.objectContaining({ id: 'test-id', name: 'Warm Tones' }))
  })
})
...
describe('updatePalette', () => {
  it('merges updates into the matching palette', () => {
    const store = makeStore()
    store.getState().hydratePalettes([makePalette({ id: 'p-1', name: 'Old Name' })])
  ...
  it('refreshes updatedAt on the matching palette', () => {
    const store = makeStore()
    store.getState().hydratePalettes([makePalette({ id: 'p-1', updatedAt: '2024-01-01T00:00:00.000Z' })])
  ...
  it('calls paletteDB.put with the updated palette', () => {
    ...
    expect(mockedPaletteDB.put).toHaveBeenCalledWith(expect.objectContaining({ id: 'p-1', name: 'New' }))
  })
})
...
describe('addPaletteComment', () => {
  it('appends the comment to the matching palette', () => {
    const store = makeStore()
    store.getState().hydratePalettes([makePalette({ id: 'p-1', comments: [] })])
  ...
  it('stores the provided comment text', () => {
    ...
    expect(store.getState().palettes[0].comments[0].text).toBe('Love this palette')
  })
  ...
  it('calls paletteDB.put with the updated palette', () => {
    ...
    expect(mockedPaletteDB.put).toHaveBeenCalledWith(expect.objectContaining({ id: 'p-1' }))
  })

  it('does not mutate non-matching palettes', () => {
    const store = makeStore()
    const paletteB = makePalette({ id: 'p-2', comments: [] })
    store.getState().hydratePalettes([makePalette({ id: 'p-1', comments: [] }), paletteB])
...
describe('updatePaletteComment', () => {
  it('updates the text of the matching comment', () => {
    ...
    store.getState().hydratePalettes([makePalette({ id: 'p-1', comments: [comment] })])
  ...
  it('refreshes comment.updatedAt', () => {
    ...
    expect(typeof store.getState().palettes[0].comments[0].updatedAt).toBe('string')
  })

  it('refreshes palette.updatedAt', () => {
    const store = makeStore()
    const comment = { id: 'c-1', text: 'Old', createdAt: '2024-01-01T00:00:00.000Z', updatedAt: '2024-01-01T00:00:00.000Z' }
    store.getState().hydratePalettes([makePalette({ id: 'p-1', updatedAt: '2024-01-01T00:00:00.000Z', comments: [comment] })])
...
describe('deletePaletteComment', () => {
  ...
  it('leaves non-matching comments in the array', () => {
    ...
    store.getState().hydratePalettes([makePalette({ id: 'p-1', comments: [commentA, commentB] })])
...
describe('setPalettesFilter', () => {
  it('updates the search field while preserving other filter fields', () => {
    ...
    expect(store.getState().palettesFilter).toEqual({ search: 'warm', groupId: 'g-1', tagIds: [] })
  })
  ...
  it('updates tagIds without resetting search or groupId', () => {
    ...
    expect(store.getState().palettesFilter).toEqual({ search: 'dark', groupId: 'g-1', tagIds: ['t-1', 't-2'] })
  })
})
...
describe('resetPalettesFilter', () => {
  it('resets the filter to the default values', () => {
    const store = makeStore()
    store.getState().setPalettesFilter({ search: 'warm', groupId: 'g-1', tagIds: ['t-1'] })
