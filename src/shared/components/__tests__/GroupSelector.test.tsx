...
beforeEach(() => {
  ...
  mockedUseAppStore.mockImplementation((selector) =>
    selector({ addGroup: mockAddGroup } as unknown as Parameters<
      typeof selector
    >[0])
  );
});
...
describe('GroupSelector', () => {
  describe('select element', () => {
    ...
    it('renders each group as an option in the select', () => {
      const groups = [
        makeGroup({ id: 'g1', name: 'Brand' }),
        makeGroup({ id: 'g2', name: 'Icons' }),
      ];
    ...
    it('reflects the selectedGroupId in the select value', () => {
      ...
      render(
        <GroupSelector {...defaultProps} groups={groups} selectedGroupId="g1" />
      );
    ...
    it('calls onSelect with the group id when a different group option is selected', async () => {
      ...
      render(
        <GroupSelector {...defaultProps} groups={groups} onSelect={onSelect} />
      );
