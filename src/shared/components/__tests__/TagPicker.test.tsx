...
beforeEach(() => {
  ...
  mockedUseAppStore.mockImplementation((selector) =>
    selector({ addTag: mockAddTag } as unknown as Parameters<
      typeof selector
    >[0])
  );
});
...
describe('TagPicker', () => {
  describe('tag buttons', () => {
    it('renders a button for each tag', () => {
      const tags = [
        makeTag({ id: 't1', name: 'Nature' }),
        makeTag({ id: 't2', name: 'Abstract' }),
      ];

      render(<TagPicker {...defaultProps} tags={tags} />);

      expect(
        screen.getByRole('button', { name: 'Nature' })
      ).toBeInTheDocument();
      expect(
    ...
    it('applies active classes to selected tags', () => {
      ...
      render(
        <TagPicker {...defaultProps} tags={tags} selectedTagIds={['t1']} />
      );
  ...
  describe('color swatches', () => {
    it('gives the scale-125 class to the swatch that was clicked', async () => {
      ...
      await userEvent.click(screen.getByRole('button', { name: '+ New tag' }));
      const blueSwatchButton = screen.getByRole('button', {
        name: 'Select color #3b82f6',
      });
