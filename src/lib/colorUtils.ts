...
export function hslToRgb({ h, s, l }: HSL): RGB {
  ...
  if (h < 60) { r = chroma; g = hueSectionValue }
  else if (h < 120) { r = hueSectionValue; g = chroma }
  else if (h < 180) { g = chroma; b = hueSectionValue }
  else if (h < 240) { g = hueSectionValue; b = chroma }
  else if (h < 300) { r = hueSectionValue; b = chroma }
  else { r = chroma; b = hueSectionValue }
...
export function normalizeHex(hex: string): string {
  const clean = hex.replace('#', '')
  if (clean.length === 3) {
    return '#' + clean.split('').map((hexChar) => hexChar + hexChar).join('')
  }
