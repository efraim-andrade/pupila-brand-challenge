...
describe('exportAllData', () => {
  it('returns all entities from all four stores', async () => {
    const { openDB } = require('idb')
    const images = [makeImage()]
    const palettes = [makePalette()]
    const groups = [makeGroup()]
    ...
    openDB.mockResolvedValue(buildMockDB({
      images: makeStore(images),
      palettes: makeStore(palettes),
    ...
    expect(result).toEqual({ images, palettes, groups, tags })
  })
})

describe('clearAndImportData', () => {
  it('clears all stores then writes snapshot data within a transaction', async () => {
    const { openDB } = require('idb')
    ...
    openDB.mockResolvedValue(buildMockDB(storeMap))

    const { clearAndImportData } = require('../db')
    const snapshot = {
      images: [makeImage()],
    ...
    await clearAndImportData(snapshot)

    expect(storeMap.images.clear).toHaveBeenCalledTimes(1)
    expect(storeMap.palettes.clear).toHaveBeenCalledTimes(1)
    expect(storeMap.groups.clear).toHaveBeenCalledTimes(1)
    expect(storeMap.tags.clear).toHaveBeenCalledTimes(1)

    expect(storeMap.images.put).toHaveBeenCalledWith(snapshot.images[0])
    expect(storeMap.palettes.put).toHaveBeenCalledWith(snapshot.palettes[0])
    expect(storeMap.groups.put).toHaveBeenCalledWith(snapshot.groups[0])
    expect(storeMap.tags.put).toHaveBeenCalledWith(snapshot.tags[0])
  })
})
